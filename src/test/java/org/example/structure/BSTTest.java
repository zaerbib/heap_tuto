...
import org.example.structure.tree.BST;
...
public class BSTTest {
    ...
    @Test
    public void remove() {
        final List<Integer> keys = IntStream.rangeClosed(0, 100).boxed().collect(Collectors.toList());
        BST<Integer> rst = new BST<>();

        keys.forEach(rst::add);
    ...
    public void height() {
        ...
        rst.add("f");
        assertEquals(3, rst.height());

        rst.add("g");
        assertEquals(4, rst.height());

        rst.add("x");
        assertEquals(5, rst.height());

        rst.add("y");
        assertEquals(6, rst.height());

        rst.add("w");
        assertEquals(6, rst.height());

        rst.add("z");
        assertEquals(7, rst.height());
